...
On a managed Mac the CLI ships inside ReportMate.app (`Contents/Helpers/reportmateutil`, symlinked to `/usr/local/bin/reportmateutil` by the app's installer); on a managed PC it sits in `C:\Program Files\ReportMate` next to the ReportMate app. Elsewhere, install it from its GitHub release (a universal macOS binary and Windows x64/arm64 builds). Then export the target and a credential:
...
An OIDC bearer token (`REPORTMATE_TOKEN`) or the shared client passphrase (`REPORTMATE_PASSPHRASE`) also work. `REPORTMATE_CLI=/path/to/reportmateutil` pins FleetMate to a specific binary; set it empty to force the HTTP path.
