...
On a managed Mac the CLI ships inside ReportMate.app (`Contents/Helpers/reportmate`, symlinked to `/usr/local/bin/reportmate` by the app's installer); on a managed PC it sits in `C:\Program Files\ReportMate` next to the ReportMate app. Elsewhere, install it from its GitHub release (a universal macOS binary and Windows x64/arm64 builds). Then export the target and a credential:
